...
@{
    ...
    overrides = @{
        'PSAvoidDefaultValueSwitchParameter' = @{
            derived = 'Switch Parameters Should Not Default To True -- Switch parameter should not default to true.'
            text = 'A switch defaulting to true cannot be turned off by omitting it -- the caller must write -Name:$false. Default it to false and invert the logic.'
        }
        ...
        'PSAvoidUsingCmdletAliases' = @{
            derived = 'Avoid Using Cmdlet Aliases or omitting the ''Get-'' prefix -- An alias is an alternate name or nickname for a cmdlet or for a command element, such as a function, script, file...'
            text = 'curl and wget mean Invoke-WebRequest in Windows PowerShell 5.1 but not in PowerShell 7, and any profile can redefine an alias. Spell the full cmdlet name.'
        }
        ...
        'PSPossibleIncorrectComparisonWithNull' = @{
            derived = 'Null Comparison -- Checks that $null is on the left side of any equality comparisons (eq, ne, ceq, cne, ieq, ine).'
            text = 'With an array on the left, -eq filters the array instead of returning a boolean, so $x -eq $null is not a null test. Put $null on the left: $null -eq $x.'
        }
        'PSShouldProcess' = @{
        ...
        'PSUseApprovedVerbs' = @{
            derived = 'Cmdlet Verbs -- Checks that all defined cmdlets use approved verbs. This is in line with PowerShell''s best practices.'
            text = 'An unapproved verb makes the command undiscoverable by Get-Command -Verb and makes Import-Module warn on every load. Pick the closest verb from Get-Verb.'
        }
        'PSUseDeclaredVarsMoreThanAssignments' = @{
            derived = 'Extra Variables -- Ensure declared variables are used elsewhere in the script and not just during assignment.'
            text = 'A variable assigned and never read is usually a typo in the name meant to read it, or a leftover from a refactor. Remove it, or fix the misspelled reader.'
        }
    ...
    entries = @{
        ...
        'PSAvoidDefaultValueSwitchParameter' = 'A switch defaulting to true cannot be turned off by omitting it -- the caller must write -Name:$false. Default it to false and invert the logic.'
        ...
        'PSAvoidUsingCmdletAliases' = 'curl and wget mean Invoke-WebRequest in Windows PowerShell 5.1 but not in PowerShell 7, and any profile can redefine an alias. Spell the full cmdlet name.'
        ...
        'PSPossibleIncorrectComparisonWithNull' = 'With an array on the left, -eq filters the array instead of returning a boolean, so $x -eq $null is not a null test. Put $null on the left: $null -eq $x.'
        ...
        'PSUseApprovedVerbs' = 'An unapproved verb makes the command undiscoverable by Get-Command -Verb and makes Import-Module warn on every load. Pick the closest verb from Get-Verb.'
        ...
        'PSUseDeclaredVarsMoreThanAssignments' = 'A variable assigned and never read is usually a typo in the name meant to read it, or a leftover from a refactor. Remove it, or fix the misspelled reader.'
