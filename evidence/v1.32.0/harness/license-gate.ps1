...
param(
    ...
    [string] $Commit = 'fb3116cab14cd8afec4e9c64ed0c2e67e76486b3',
    [string] $Out = 'C:\Users\mande\AppData\Local\Temp\psl-267\out',
    [switch] $RedControl
)
...
function Test-Gate {
    ...
    foreach ($d in $docs) {
        ...
        $namesStart = ($t -match ('from v' + [regex]::Escape($ver))) -or ($t -match ('effective from v' + [regex]::Escape($ver)))
        $namesBand  = ($t -match 'v1\.6\.1 through v1\.31\.2')
        $ok = $namesStart -and $namesBand
        $checks += @{
            name = ($d + ': names the Apache-2.0 start version and the fixed GPL band')
            ok = $ok
            detail = ('names-v' + $ver + '=' + $namesStart + ' names-band=' + $namesBand)
        }
        if (-not $namesStart) { $findings += ($d + ' does not name v' + $ver + ' as the Apache-2.0 start') }
        if (-not $namesBand)  { $findings += ($d + ' does not bound the GPL band at v1.31.2') }
    }
