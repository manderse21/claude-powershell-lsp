...
| **Capture-by-default posture (R-A)** | Whether diagnostic capture -- which writes **real source snippets** -- should stay on by default while `enableStats`, which records only timings, is opt-in. Scored TRUE against disk by dispatch 000277 ([`ENTERPRISE-PROGRAM-DOCKET.md`](ENTERPRISE-PROGRAM-DOCKET.md) C1): `Add-DiagnosticCaptureEntries` is called outside every `$StatsOn` guard in `scripts/lsp-client.ps1`, and no separate knob exists. The asymmetry is real and is not declared in `CONTRACT.md` or `docs/configuration.md`. **The docket recommends declaring it rather than gating it, and rules nothing** |
| **`orgPolicy` degrade direction (R-B)** | Whether a configured org policy that is missing, non-absolute, unreadable or integrity-failed should keep degrading **open** -- no exclusions applied, reason logged, doctor check 10 surfacing it -- or gain an opt-in strict mode. Scored TRUE against disk by dispatch 000277 ([`ENTERPRISE-PROGRAM-DOCKET.md`](ENTERPRISE-PROGRAM-DOCKET.md) C2) across four degrade paths in `Import-OrgPolicyExcludes`. The payload only *removes* diagnostics, so failing open cannot hide a defect -- which is the argument for leaving it, and is a recommendation, not a decision |
