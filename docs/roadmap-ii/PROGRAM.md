...
| **R2-05** | Corpus licensing and provenance audit, then the corpus commons un-gate | 000222, 000251 | **HUMAN-GATED** | **Un-gated.** The audit passed the Arc B licensing gate (D1), and the relicensing decision deferred to Arc B activation has been made: the corpus publishes under the project license, **Apache-2.0** -- one license across the repository. The corpus is documented in-repo for outside use, reproduction, and citation. What remains is **external publishing, which is maintainer-owned**; this row claims no announcement or submission | [`CORPUS-PROVENANCE-AUDIT.md`](CORPUS-PROVENANCE-AUDIT.md); manderse21/claude-powershell-lsp#147, merge `c0e4b51`; D1; [`corpus.md`](../corpus.md) and the dispatch 000251 entry in [`decision-ledger.md`](../decision-ledger.md) (its PR and merge SHA land with the next edit that touches this file, on the R2-15 precedent) |
