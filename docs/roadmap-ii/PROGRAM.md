...
| --- | --- |
| **RE-OPEN T6.1 -- the capture default (R-A)** | **Ruled (a)**, 2026-09-05, as **R8**, ratified by acceptance of the 000279 inbox: *"Re-open T6.1 narrowly: a metadata-only capture mode on the POWERSHELL_LSP_* admin env surface, T6.1 amended to record the management-plane reader the review named (EDR, backup, eDiscovery, DLP). NOT built tonight -- chartered for night 4, because the capture's consumer set deserves its own derivation."* **BUILT by dispatch 000282** as `POWERSHELL_LSP_CAPTURE_MODE` (`full` / `metadata` / `off`) at `Add-DiagnosticCaptureEntries`, with the hash computed from the offending line in every mode so the corpus channel is preserved, and `message` dropped in `metadata` on a measurement showing PSScriptAnalyzer quotes source identifiers into it. `THREAT-MODEL.md` T6.1 is **amended at all five of its sites, narrowed and not withdrawn**: the local-reader acceptance stands verbatim and the management-plane reader is recorded beside it. Ruling **R19** added the fleet-visible half, `captureMode` in the `doctor -Json` envelope |
| **Pull P1-4 forward -- the protocol handshake (R15)** | **Ruled build-now and BUILT**, 2026-09-06, as **R15**, ratified by acceptance of the 000282 inbox: *"Build the protocol handshake now. It is zero freeze exposure on both frozen surfaces, the docket prices it at 3-4 hours today and materially more once P1-2 ships a second consumer, and it is the ordering finding the review did not draw: P1-4 must lead P1-2."* Built by dispatch **000282** as `protocolVersion` + `capabilities` on request and response; absent means 1, an unknown version is answered rather than refused. The docket named one request-building site and a census found **three**; all three announce it, and all five daemon response paths were routed through one write seam |
| **Pull P2-2 forward -- the assurance pack (R16)** | **Ruled write-now and BUILT**, 2026-09-06, as **R16**: *"Write the assurance-pack document. No code, zero freeze, and it names a set that already exists frozen. It must say what it is not."* Built by dispatch **000282** as [`docs/assurance-pack.md`](../assurance-pack.md), linked from the `ruleset` knob. The count was **derived from the emitters** and the docket's six is correct; the eligibility test is that the judgment cannot be reached from the file's own AST |
| **The gallery-fallback end-to-end test question (R17)** | **Ruled: the structural proof is the stopping point**, 2026-09-06, as **R17**: *"A CI-only test that fetches the 14.6 MB pinned artifact on every run buys one end-to-end assertion at the price of a network dependency in the suite that proves the plugin works offline. If a live fetch is ever wanted it is a scheduled job, not a PR gate, and it is not chartered."* This answers the first of the two questions dispatch 000279 asked back. **Nothing is built for it** |
| **Verbatim rulings as a standing convention (R18)** | **Ruled standing**, 2026-09-06, as **R18**: *"Every charter carries every ruling it executes or establishes verbatim in its own body, in a section titled for it, so the ruling is attributable from disk without the chat that produced it."* This answers the second question dispatch 000279 asked back -- the one 000278 raised when R2, R3 and R6 proved unrecoverable from disk. It is a **drafting rule for Strategic-Claude**, recorded once in the decision ledger by dispatch **000282**, which complies with it |
| **A fleet-visible half for P0-2 (R19)** | **Ruled build-now and BUILT**, 2026-09-06, as **R19**: *"A control the fleet cannot verify is half a control."* Built by dispatch **000282** as `captureMode` in the `doctor -Json` envelope -- resolved mode, raw environment value, recognized flag. All three, because an unrecognized value resolves to `full` and without `raw` and `recognized` a misspelled fleet-deployed value would be indistinguishable from a host deliberately left at the default. Additive: `schemaVersion` stays 1, and the dispatch wrote the previously-absent policy saying so |
| **RE-OPEN T4.2 -- the `orgPolicy` payload (R-B)** | **Ruled (b)**, 2026-09-05, as **R9**: *"Charter Policy v2's include-side payload only; the signing half waits until a trust root exists, per THREAT-MODEL T4.1's own words. P1, not tonight."* The payload half is chartered as P1-5's first half; the signing half stays blocked on the trust root T4.1 says the mechanism does not have. Nothing is built |
| **`gallery-fallback` hash-chain gap (R-C)** | **Ruled (a) and BUILT**, 2026-09-05, as **R10**: *"Verify the gallery-fallback bytes against the same pin and fail closed."* Built by dispatch **000279**: the fallback now acquires the `.nupkg` itself over PackageManagement and feeds the same single `Test-PinnedFileHash` gate every other layer feeds, failing closed identically. There is no acquisition route left in either `ensure-*` script whose bytes the pin does not verify. The option-(b) opt-in shape was not taken and needs no successor |
| **Doctor-surface docket (R-D)** | **Ruled (a) and BUILT**, 2026-09-05, as **R11**: *"Build P0-1a+b+c as one dispatch."* Built by dispatch **000279** as `doctor -Json`, the `HEALTHY` / `DEGRADED` / `UNHEALTHY` / `UNPROVEN` envelope, and the opt-in `-RequireProven` gate. [`DOCTOR-SURFACE-DOCKET.md`](DOCTOR-SURFACE-DOCKET.md)'s S3 -- the ephemeral daemon for check 11 -- is **not** built and stays P3 with its own charter and its own ruling still owed, because it is the one slice that would make a report-only surface start a process |
| **T3 spread-basis re-ratification (R-F)** | **Ruled (c) -- decide later**, 2026-09-05, as **R12**: *"Decide T3's spread basis after the quiet-host re-runs. Not tonight; the re-runs are a human leg on a quiet host."* The question is not answered and is not deferred indefinitely either: it is ordered behind a measurement only a human can take. The **T3 post-release miss** row above stays open and is the separate question |
| **Claude Code Current-1: Required or Advisory (R-G)** | **Ruled Advisory**, 2026-09-05, as **R13**: *"Claude Code Current-1 is Advisory, not Required. Recorded; P1-3 builds later."* The CI leg is chartered as advisory when P1-3 is built; nothing is built here and the four CI leg identities are unchanged |
| **The three standing rulings R2 opened (R14)** | **No re-rule**, 2026-09-05: *"No re-rule on the custom-rule seam, the native-LSP gate, or PS 5.1 first-class. The docket argued all three both ways and recommended none; the seam's re-open condition is real user demand and a review is not demand."* [`ENTERPRISE-PROGRAM-DOCKET.md`](ENTERPRISE-PROGRAM-DOCKET.md) section 5 carries the argument on both sides and is left exactly as written |
| **The enterprise review audit itself (R-E)** | **Answered by execution, not deferred.** Dispatch **000278** confirmed the anchored file exists -- `C:\Users\mande\Downloads\enterprise-review-2026-09-05.md`, 29,569 bytes, SHA-256 `380CFA18059EEF598BA867268BD0DEAEF438CFD380F836A44F33E8C2ACB372AD` -- read it end to end, and scored every numbered item 1-17, the five things and every row of its phase table in [`ENTERPRISE-PROGRAM-DOCKET.md`](ENTERPRISE-PROGRAM-DOCKET.md) section 3. Section 5 now argues all three standing rulings both ways and recommends re-opening none of them. No successor needs minting for the audit itself |
