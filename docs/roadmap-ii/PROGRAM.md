...
| **RE-OPEN T6.1 -- the capture default (R-A)** | Whether an accepted finding is re-opened by a reader set its acceptance did not consider. `THREAT-MODEL.md` T6.1 accepts capture-on-by-default with the reason that the exposure runs "to a local user who already has the source files it quotes". The external enterprise review of 2026-09-05, audited claim by claim by dispatch **000278**, names readers for whom that clause is false -- EDR, backups, forensic collection, eDiscovery, DLP, disk imaging and endpoint management -- which is a claim that T6.1 mis-scoped the reader set, not a restatement of it. Dispatch 000277's leg C already closed the *local* half by containing every plugin-created POSIX object. [`ENTERPRISE-PROGRAM-DOCKET.md`](ENTERPRISE-PROGRAM-DOCKET.md) ruling **R-A** carries three options and recommends a metadata-only capture mode on the ruled `POWERSHELL_LSP_*` admin surface (slice P0-2, zero freeze exposure). **Nothing is built, and T6.1 is unamended** |
| **RE-OPEN T4.2 -- the `orgPolicy` payload (R-B)** | Whether `orgPolicy` gets an include-side layer. T4.2 accepts fail-open with its reason, and the review's argument is **not the one T4.2 answered**: it says a subtract-only payload (`ExcludeRules` and nothing else) cannot express an organization *requirement* at all, so the degrade direction was the wrong axis to have ruled on. Scored TRUE against disk by dispatch 000278 ([`ENTERPRISE-PROGRAM-DOCKET.md`](ENTERPRISE-PROGRAM-DOCKET.md) item 2). The cost driver is `THREAT-MODEL.md` T4.1's own words -- a signed policy needs "a trust anchor the org policy mechanism does not have" -- which is why ruling **R-B** recommends splitting the payload half from the signing half rather than chartering Policy v2 whole |
| **T3 spread-basis re-ratification (R-F)** | Whether T3's ratified spread basis moves from N=15 to N=45. This is a **change to a ratified target**, so no survey and no runner may take it: [`T3-REGRESSION-SURVEY.md`](T3-REGRESSION-SURVEY.md) raises it as its proposal P3 and explicitly puts it out of its own scope. Its force is that section 9 of [`SLO-BASELINES.md`](SLO-BASELINES.md) reads spread zero as making a miss a behavioural regression, and that reading rests on 15 observations. The survey's own order is **quiet-host re-runs first, then re-read**, so ruling **R-F**'s recommended default is to decide after them. Related to, and not the same question as, the T3 post-release miss row above |
| **Claude Code Current-1: Required or Advisory (R-G)** | Whether a compatibility-certification CI leg against the previous client version blocks a release or only advises. Surfaced by dispatch 000278 costing the review's item 4 as slice P1-3, where the compatibility *declaration* is the output of a matrix rather than a promise made ahead of one -- which is what keeps it consistent with [`SUPPORT-POLICY.md`](../SUPPORT-POLICY.md)'s refusal to declare an untested floor. Ruling **R-G** recommends Advisory: a Required leg that cannot obtain an older client blocks every release, trading one enterprise risk for a worse one |
