...
**P0-2 -- metadata-only capture mode (review item 1; ruling R-A). BUILT by dispatch 000282, legs A-E.**
...
**P1-4 -- protocol version and capabilities handshake (review item 17). BUILT by dispatch 000282, leg F, under ruling R15.**
...
| --- | --- | --- | --- | --- |
| **P2-1 OTel export** (item 7) | Render existing instruments -- the `enableStats` JSONL timing lines and the capture `hash` -- as OTel metrics behind a `POWERSHELL_LSP_OTEL_ENDPOINT` env var, metadata only | ~2-3 d | **ZERO** on the ruled env surface | Cheaper than the review assumes: the measurements exist. Do **not** ship before P0-2 rules on what may leave the host. **TIMING HALF BUILT by dispatch 000290.** `scripts/export-otel.ps1` plus `scripts/lib/otel-common.ps1` render `logs/stats.jsonl` as OTLP/HTTP JSON behind `POWERSHELL_LSP_OTEL_ENDPOINT`; five metrics, zero new measurement, nothing on the edit path (the exporter is an out-of-band reader, the shape `show-stats.ps1` already is). **P0-2's precondition was met before building** -- R-A was ruled and P0-2 BUILT by 000282, which is what this row required. THE BOUNDARY IS AN ALLOWLIST (`ext`, `taken`, `cached`), NOT A DENYLIST OVER `path`, and that is the decision the slice turns on: a stats row carries the absolute path of the edited file, and a rule that stripped one named field would keep passing the day a second path-bearing field joins the row. The suite asserts the CLASS -- an unknown `settingsPath` is put on a row and required absent -- not the field. RED control: no prior implementation exists (new file), so the control is the NAIVE FIRST-CUT renderer that copies the row's own fields, recorded honestly as the weaker warrant it is; it overrides exactly ONE function, which gives it a SURVIVING ARM (the gauges and counters are shipped code in both runs and must come out identical, so a fixture that broke the renderer instead of defeating the boundary is caught rather than credited). Proven to BITE against the SHIPPED path as well: widening the real allowlist to include `path` turns **6** tests red. The endpoint resolver's fallback is DELIBERATELY INVERTED from `POWERSHELL_LSP_CAPTURE_MODE`'s -- a typo there resolves to `full` because nothing may gate the capture channel; a typo here resolves to OFF because the permissive direction is a network egress to a destination nobody named. Credentials are never printed (userinfo and query redacted from every message including failures; an unparseable value is not echoed at all). 37 tests, green under BOTH pwsh 7 and Windows PowerShell 5.1. **DOCTOR HALF BUILT by dispatch 000291.** The `doctor -Json` envelope carries `otelExport` -- `display` (userinfo and query redacted), `configured`, `recognized` -- so a management plane can ask a host whether it is shipping metrics and where to, instead of reading the collector's inbox and inferring which hosts are missing (an inference that cannot separate *not configured* from *configured and failing*). 000290's deferral reason is DISCHARGED: it deferred this because the field lands in `DoctorJson.Tests.ps1`, whose guards were under adjudication in that same dispatch, and 000290 repaired them. **THE PUBLISHED SET IS AN ALLOWLIST OF THREE NAMES, NOT THE RESOLVER MINUS TWO** -- `Get-OtelEndpointReportInfo` owns that boundary as ONE function (Hub Rule 18), the same shape `Get-OtelAttributeAllowList` owns for a stats row; `endpoint` carries credentials verbatim, and `raw`, when `recognized` is false, did not parse at all, so nothing has inspected it and nothing can promise it holds no secret. The suite asserts the published key set is exactly `display,configured,recognized` -- testing the CLASS, not the two known-bad fields, because a denylist keeps passing the day a third credential-bearing field joins the resolver. RED control: **no prior implementation exists** (the field is new), so the control is the NAIVE FIRST-CUT projection that publishes the resolver verbatim, recorded honestly as the weaker warrant that is; it is pinned by an LF-normalized SHA-256 so drift is caught, and it overrides exactly ONE function, which gives it a SURVIVING ARM (`captureMode`, the derived status, the summary counts and the check array are shipped code in both runs and must come out identical). Proven to BITE against the SHIPPED path and not only against the fixture: mutating the real projection to the naive form turns **4** tests red while **45** stay green. The two controls' fallback directions are asserted TOGETHER in one test, so neither can quietly be brought into line with the other. `schemaVersion` does not move -- the second field added under 000282's additive policy and the first merely to follow it. **49** tests in the file, green under BOTH pwsh 7.6.5 and Windows PowerShell 5.1. **CAPTURE HALF BUILT by dispatch 000291 -- P2-1 IS NOW COMPLETE, both halves and the doctor surface.** A sixth metric, `powershell_lsp.diagnostics.shapes`, counts DISTINCT diagnostic shapes off `dogfood/diagnostics.jsonl`, split by `ruleId` / `severity` / `source`. It answers what the volume counters cannot: `diagnostics.records` cannot tell one rule firing four hundred times from four hundred distinct problems. **THE `hash` IS WHAT THE METRIC COUNTS AND IT NEVER LEAVES** -- one point per distinct hash is unbounded cardinality and an event log wearing a metric's clothes, the same reason `ts` is off the stats list; `snippet` (verbatim source), `message`, `file`, `line` and `col` are off the capture allowlist for the plainer reason that they are source and locations. **THE ALLOWLIST WAS EXTENDED, NOT ROUTED AROUND, AND EXACTLY ONE FUNCTION STILL OWNS IT:** `Get-OtelAttributeAllowList -Kind` SELECTS A LIST RATHER THAN ADDING A DOOR -- the two record kinds have disjoint vocabularies, so a flat union would have permitted a capture field on a stats row and the reverse, inert only until one record gained a field named like the other's, which is the very shape an allowlist exists to refuse. An unrecognized kind publishes NOTHING rather than falling back to another kind's permissions, asserted in its own test. The stats rendering is proven UNCHANGED BYTE-FOR-BYTE against the pre-change payload. With no capture log **no shapes metric is emitted at all, never a zero** -- a zero would tell a fleet dashboard this host produced no distinct diagnostics rather than that the reader was handed no log, the same conflation the exporter already refuses on the stats side. RED control: **no prior implementation exists**, so it is the NAIVE FIRST CUT that emits one point per hash carrying the hash as an attribute -- a mutant that reads as MORE informative than the shipped version, which is what makes it the right one -- pinned by LF-normalized SHA-256 and overriding exactly ONE function, giving it a SURVIVING ARM (all five of 000290's metrics are shipped code in both runs and asserted identical). Proven to BITE against the SHIPPED path: widening the real capture allowlist to include `hash` and `snippet` turns **8** tests red while **42** stay green. The unknown-field probe is the same `settingsPath` 000290 used on the stats side, so both halves test the same property rather than two similar ones. **50** tests in the file (37 pre-existing, every one still green), green under BOTH pwsh 7.6.5 and Windows PowerShell 5.1 |
| **P2-2 name the assurance pack** (item 12) | A document: the six plugin-owned rules as a named pack, with the eligibility test that makes a rule belong to it | ~3-4 h, no code | **ZERO** -- naming a set that already exists frozen | The genuinely cheap half of item 12. The *signed org extensions* half is Pillar H and is not proposed. **BUILT by dispatch 000282, leg G**, under ruling R16: `docs/assurance-pack.md`. The count was derived from the emitters and the six above is correct |
| **P2-3 container CI leg** (item 16) | A fifth CI leg: Ubuntu container, non-root, no TTY, no profile, running the suite plus `doctor -RequireProven` | ~1-2 d | ZERO | **Downstream of P0-1.** A container leg that cannot assert health asserts nothing. **BLOCKER REMOVED by dispatch 000285 (probe half BUILT):** check 1 read the executable's file-version *resource*, a Windows-only artifact, so on Linux it answered *"found pwsh 0.0.0.0 but PowerShell 7+ is required"* on a host running 7.4.2 -- `doctor -RequireProven` could never have passed in the container, and the leg would have gated on a fabricated failure. The probe now prefers `$PSVersionTable.PSVersion` in-process, guarded by an IsSelf test so a *different* `pwsh` on PATH is never reported at this process's version, and a `0.0.0.0` with no in-process answer degrades to UNKNOWN rather than to a fabricated `fail`. RED control: the prior implementation, on the Linux shape, is asserted to produce the wrong answer, and mutating the fix out turns exactly the two fix-asserting tests red. **BUILT by dispatch 000286:** the leg is `container-pwsh`, its OWN job with its OWN name -- the `pester` job is named `${{ matrix.label }}`, so a fifth matrix entry would have re-keyed the four existing legs and RENAMED their check contexts, silently removing required checks; the first 165 lines of the workflow are byte-for-byte unchanged and the diff is 102 insertions, 0 deletions. Non-root, no TTY, no profile, Pester installed in-job, image pinned BY DIGEST. Two findings the build had to make first, both measured rather than assumed: (1) the official image ships **no git** (nor curl, dotnet, unzip), so the checkout happens on the HOST and the workspace is mounted -- under a job-level `container:`, `actions/checkout` finds no git and falls back to a REST tarball, yielding a tree with no `.git` at all; (2) `powershell:latest` is **7.4.2** while the PSScriptAnalyzer 1.25.0 pin this repo carries refuses to import below **7.4.6**, so on `latest` the analyzer never loads and the corpus tier silently returns EMPTY findings -- 41 corpus and 13 integration failures, every one an artefact of the image, against **266/0 and 66/0** on `7.5-ubuntu-24.04`. `doctor -RequireProven` CANNOT pass in a container and no change to this repository can make it (six of its eight unknowns require a live plugin subprocess, one reports the shipped offline default, one is unknown BY DESIGN at default `ps_host`; setting `CLAUDE_PLUGIN_DATA` to a writable directory makes it worse, not better -- measured UNHEALTHY at pass 7 / fail 2 / unknown 5). So the leg asserts the CENSUS explicitly: zero failures, check 1 naming the REAL in-process version, the exact unknown set, and `-RequireProven` exiting 2 rather than 0 or 1. RED control: restoring the pre-000285 probe turns it red on seven counts, including `found pwsh 0.0.0.0` on a 7.5.0 host. **Remaining gap, named:** the census's six session-dependent unknowns can only be proven from inside a live Claude Code plugin subprocess, which no container leg can stage; proving them needs P1-1's human measurement or an in-session harness, not this leg |
...
| --- | --- | --- | --- | --- |
| **R-A** | **RE-OPEN T6.1?** The review's EDR / backup / eDiscovery / DLP reader set falls outside T6.1's stated reason ("a local user who already has the source files it quotes"). 000277's leg C already closed the local half | (a) re-open narrowly and build **P0-2** as a metadata-only mode on the admin env surface, T6.1 amended to record the management-plane reader; (b) leave T6.1 accepted as written and restate the acceptance in `CONTRACT.md` / `docs/configuration.md`; (c) re-open fully and make metadata the default | **(a)** -- it answers the one clause the new argument reaches, keeps the dogfood channel T6.1 was protecting, and costs zero freeze exposure on the env surface | **(a)**, ruled by Mike 2026-09-05 as **R8**, ratified by acceptance of the 000279 inbox. Verbatim: *"R8 (R-A) = (a). Re-open T6.1 narrowly: a metadata-only capture mode on the POWERSHELL_LSP_* admin env surface, T6.1 amended to record the management-plane reader the review named (EDR, backup, eDiscovery, DLP). NOT built tonight -- chartered for night 4, because the capture's consumer set deserves its own derivation."* **BUILT by dispatch 000282** |
| **R-B** | **RE-OPEN T4.2?** The review's argument is not the one T4.2 answered: it says a subtract-only payload cannot express an org requirement, so the degrade *direction* was the wrong axis to rule on | (a) charter **P1-5** Policy v2 whole (include-side payload first, signing second); (b) charter the payload half only and defer signing until a trust root exists; (c) leave T4.2 as accepted and record the payload gap in `docs/configuration.md` | **(b)** -- the payload argument stands on its own, and the signing half needs a trust root `THREAT-MODEL.md` T4.1 says the mechanism does not have. Splitting keeps the cheap half from waiting on the expensive one | **(b)**, ruled by Mike 2026-09-05 as **R9**, ratified by acceptance of the 000279 inbox. Verbatim: *"R9 (R-B) = (b). Charter Policy v2's include-side payload only; the signing half waits until a trust root exists, per THREAT-MODEL T4.1's own words. P1, not tonight."* |
| **R-C** | For **P0-3**, which shape closes the `gallery-fallback` hash gap? | (a) verify the fallback's bytes against the same pin, fail closed; (b) fail closed on the fallback by default with an explicit opt-in to accept publisher-integrity-only bytes | **(a)** -- zero freeze exposure, and it makes the one unpinned route match the other four. Carried unchanged from the 000277 skeleton | **(a)**, ruled by Mike 2026-09-05 as **R10**, ratified by acceptance of the 000279 inbox. Verbatim: *"R10 (R-C) = (a). Verify the gallery-fallback bytes against the same pin and fail closed. BUILT TONIGHT as legs E and F."* **BUILT by dispatch 000279** |
| **R-D** | `DOCTOR-SURFACE-DOCKET.md` has been unruled since 000276. Build **P0-1**? | (a) P0-1a+b+c as one dispatch; (b) P0-1a only; (c) none | **(a)** -- that docket's own section 5 recommends S1, the review independently reaches the same item, and the vocabulary (P0-1b) is what makes the JSON usable by a fleet tool | **(a)**, ruled by Mike 2026-09-05 as **R11**, ratified by acceptance of the 000279 inbox. Verbatim: *"R11 (R-D) = (a). Build P0-1a+b+c as one dispatch. BUILT TONIGHT as legs A through D."* **BUILT by dispatch 000279** |
